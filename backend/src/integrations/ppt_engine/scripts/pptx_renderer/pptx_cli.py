"""CLI entry point for pptx_renderer."""
...
if __package__ in {None, ''}:
    ...
    package = types.ModuleType('pptx_renderer')
    package.__path__ = [str(Path(__file__).resolve().parent)]  # type: ignore[attr-defined]
    sys.modules.setdefault('pptx_renderer', package)
    __package__ = 'pptx_renderer'
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description='internal rendering library - SVG to PPTX Tool (Office Compatibility Mode)',
...
''',
    ...
    if gen_native:
        if verbose:
            print("internal rendering library - SVG to PPTX Tool")
            print("=" * 50)
    ...
    if gen_legacy:
        if verbose:
            if gen_native:
                print()
                print("-" * 50)
            print("internal rendering library - SVG to PPTX Tool (SVG Reference)")
            print("=" * 50)
            print(f"  Project path: {project_path}")
