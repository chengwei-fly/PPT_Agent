...
Common technical constraints for internal rendering library, eliminating cross-role file duplication.
...
python3 scripts/pptx_renderer.py <project_path>
...
python3 scripts/pptx_renderer.py <project_path> --recorded-narration audio
```
...
Best for: cards, floating panels, elevated elements. The `pptx_renderer` converter automatically converts `feGaussianBlur` + `feOffset` into native PPTX `<a:outerShdw>`.
